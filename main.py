...
out_data = {}
# Члены комиссии
# букин,скворцов, блиновсков - председатель
comm_members = ['107942', '2014498', '2881519', '2881519', ]

RD = {'НГДО': {'п.3': ['111540', '111543', '111544', '111545'], 'п.4': [], 'п.13': [], 'п.10': ['70573']},
      'СК': {'п.1': ['86341', '86384', '86386', '86398', '86399'], 'п.2': [], 'п.3': []},
      'КО': {'п.1': ['112685', '112686', '112687', '112688'], 'п.2': [], 'п.3': []},
      'ГО': {'п.1': ['112032', '112035', '112036', '112037'], 'п.2': [], 'п.3': []},
      'ГО+КО': {'п.1': ['124652', '124654', '124655', '124656']},
      'СК+ГО': {'п.1': ['112032', '112035', '112036', '112037', '86341', '86384', '86386', '86398', '86399']},
      'СК+КО': {'п.1': ['112685', '112686', '112687', '112688', '86341', '86384', '86386', '86398', '86399']},
      'ГО+КО+СК': {'п.1': ['124652', '124654', '124655', '124656', '86341', '86384', '86386', '86398', '86399']}}

G = {'НГДО': {'п.3': ['111762', '111763', '111764'], 'п.4': []},
     'ГО': {'п.1': ['112038', '112039', '112040', ], 'п.2': [], 'п.3': []}}

RAD = {'НГДО': {'п.3': ['111777', '127832', '127833', '127836', '127834', '111778', '111779', '127840', '127841', ],
                'п.4': []}, }

MPS = {'НГДО': {'п.3': ['70685', ]}}
MP = {'НГДО': {'п.3': ['82534', ]}}
NI = {'ГО': {'п.2п': ['82273']}}
ZN = {'ГО': {'п.2п': {'Т+О': ['82277'], 'Т+М+Т': ['82276']}}}
APG = {'НГДО': {'п.3': ['91723']}}
T = {'НГДО': {'п.3': ['71336', '71335', '71332', '71334', '71337', '71338']}}
...
def search_blueprint(dict, group):
    out_blueprint = []
    if dict == ZN:
        if 'Т+О' in dict['ГО']['п.2п'].keys():
            out_blueprint = dict['ГО']['п.2п']['Т+О']
        else:
            out_blueprint = dict['ГО']['п.2п']['Т+М+Т']
    elif dict == RD:
        if len(group.keys()) == 1:
            for vid in group.keys():
                for numbrs in group[vid].split(','):
                    out_blueprint += dict[vid][numbrs]
        elif len(group.keys()) == 3:
            out_blueprint = dict['ГО+КО+СК']['п.1']
        else:
            if 'ГО' in group.keys() and 'КО' in group.keys():
                out_blueprint = dict['ГО+КО']['п.1']
            elif 'ГО' in group.keys() and 'СК' in group.keys():
                out_blueprint = dict['СК+ГО']['п.1']
            elif 'КО' in group.keys() and 'СК' in group.keys():
                out_blueprint = dict['СК+КО']['п.1']
    else:
        for vid in group.keys():
            for numbrs in group[vid].split(','):
                out_blueprint += dict[vid][numbrs]
